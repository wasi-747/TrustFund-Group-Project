...
const AdminDashboard = () => {
  ...
  const handleVerification = async (id, status) => {
    try {
      ...
      toast.success(status === "approved" ? "User Verified!" : "Request Rejected.");
  ...
  const handleMilestoneAction = async (campaignId, milestoneId, action) => {
    try {
      const token = localStorage.getItem("token");
      const endpoint = action === "approve" ? "/api/admin/milestone/approve" : "/api/admin/milestone/reject";
      ...
      const successMsg = action === "approve" ? "💰 Funds Released!" : "❌ Milestone Rejected.";
  ...
  if (loading)
    return <div className="text-center mt-20 text-emerald-500 animate-pulse font-bold">Loading...</div>;
        ...
        <div className="space-y-6">
          {activeTab === "verifications" && (
            <div className="grid gap-6">
                 {requests.length === 0 ? (
                  <p className="text-gray-500 text-center py-20">No pending requests.</p>
                ) : (
                  requests.map((req) => (
                    <div
                      key={req._id}
                      className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-2xl flex justify-between items-center shadow-sm"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-xs bg-yellow-500/10 text-yellow-500 px-2 py-0.5 rounded mt-1 inline-block">
                      ...
                      <div className="flex gap-4">
                        <button
                          ...
                          className="px-4 py-2 text-sm font-bold text-red-500 bg-red-500/10 rounded-lg hover:bg-red-500/20"
                        >
                          Reject
                        </button>
                        <button
                          onClick={() => handleVerification(req._id, "approved")}
                          className="px-4 py-2 text-sm font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-500"
                        >
                          Verify User
                        </button>
                      </div>
          ...
          {activeTab === "milestones" && (
            <div className="space-y-8">
                {Object.keys(groupedMilestones).length === 0 ? (
                  <p className="text-gray-500 text-center py-20">No pending milestone payouts.</p>
                ) : (
                  Object.entries(groupedMilestones).map(([campaignId, { title, items }]) => (
                    <div key={campaignId} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
                        {/* CAMPAIGN HEADER */}
                        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                            <h3 className="font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">
                                {title}
                                <span className="text-xs bg-emerald-500 text-white px-2 py-0.5 rounded-full">{items.length} Requests</span>
                            </h3>
                        </div>
                                ...
                                <div key={idx} className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                    <div>
                                        <div className="flex items-center gap-3 mb-1">
                                            <h4 className="font-bold text-base text-gray-800 dark:text-gray-200">{item.milestone.title}</h4>
                                            {item.milestone.status === "pending_approval" && (
                                        ...
                                        <p className="text-sm text-gray-500 max-w-xl">{item.milestone.description}</p>
                                    ...
                                    <div className="flex gap-3 w-full md:w-auto">
                                        ...
                                        {/* REJECT */}
                                        <button
                                            onClick={() => handleMilestoneAction(campaignId, item.milestone._id, "reject")}
                                        ...
                                        {/* APPROVE */}
                                        <button
                                            onClick={() => handleMilestoneAction(campaignId, item.milestone._id, "approve")}
            ...
            {proofModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <h3 className="text-xl font-bold mb-1 text-white">Proof of Work</h3>
                        <p className="text-emerald-400 text-sm font-bold mb-4">{proofModal.title}</p>
                        ...
                        <div className="flex-1 overflow-y-auto min-h-0 bg-black/50 rounded-lg p-2 mb-4 border border-gray-800 flex items-center justify-center">
                            <img src={proofModal.url} alt="Proof" className="max-w-full max-h-[60vh] object-contain rounded" />
                        </div>
                        
                        <div className="bg-gray-800 p-4 rounded-xl">
                            <p className="text-gray-400 text-xs uppercase font-bold mb-1">Description</p>
                            <p className="text-gray-200 text-sm">{proofModal.desc}</p>
                            <p className="text-gray-500 text-xs mt-2 text-right">Submitted: {new Date(proofModal.date).toLocaleString()}</p>
                        </div>
                    </motion.div>
                </div>
            )}
