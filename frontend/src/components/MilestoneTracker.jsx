...
const MilestoneTracker = ({ milestones, releasedAmount, campaignId, isOwner, onRefresh }) => {
  ...
  React.useEffect(() => {
    ...
    socket.on("milestone_updated", (data) => {
        // If this update belongs to THIS campaign
        if (data.campaignId === campaignId) {
            if (data.status === "approved") {
                toast.success(`🎉 Milestone Approved! Funds released.`);
            } else if (data.status === "rejected") {
                 toast.error(`❌ Milestone Proof Rejected.`);
            }
  ...
  const handleSubmitProof = async (e) => {
    ...
    if (!proofFile || !proofDesc) {
        return toast.warning("Please provide both an image/file and a description.");
    }
      ...
      {showProofModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-900 w-full max-w-md p-6 rounded-2xl relative border dark:border-gray-800">
                ...
                <h3 className="text-xl font-bold mb-4 dark:text-white">Submit Proof of Work</h3>
                <p className="text-sm text-gray-500 mb-4">
                    Upload an image (receipt, photo of work) and describe what you did to request release of funds for <b>{selectedMilestone?.title}</b>.
                </p>

                <form onSubmit={handleSubmitProof} className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Proof Image</label>
                        <input 
                            type="file" 
                            accept="image/*"
